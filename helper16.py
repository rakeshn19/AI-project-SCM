...
def plotFreq(data, column_list, cutoff=-1):
    ...
    """
    # Calculate the number of rows needed for subplots based on the number of columns to plot
    num_cols = len(column_list)
    num_rows = np.ceil(num_cols / 2).astype(int)
    
    # Create figure with dynamic size based on the number of plots
    plt.figure(figsize=(12, num_rows * 4))
    
    for i, col in enumerate(column_list, 1):
        plt.subplot(num_rows, 2, i)
        # Calculate the number of unique values for the title
        unique = data[col].value_counts().shape[0]
        tit = f"{col.upper()} Freq of {unique} Unique"
        
        # Plotting
        if cutoff == -1:
            data[col].value_counts().plot(kind='bar')
        else:
            data[col].value_counts().iloc[:cutoff].plot(kind='barh')
        
        plt.title(tit, loc='right')  # Adjust title location if needed
...
def parse_raw_data(ExcelFileObject):
    ...
    summary =  pd.read_excel(full, sheet_name=full.sheet_names[0])#full.parse(sheetname = full.sheet_names[0])
    summary.name='summary'
    # Purpose data
    purpose =  pd.read_excel(full, sheet_name=full.sheet_names[1])
    purpose.name = 'purpose'
    # Ref is the data dictionary for reference on what the columns mean
    ref =  pd.read_excel(full, sheet_name=full.sheet_names[2]).iloc[:33,:]
    ref.name = 'ref'
    # Data if the full data set
    data =  pd.read_excel(full, sheet_name=full.sheet_names[3])
    data.name = 'data'
...
def get_blocks_by_dtype(data):
    ...
    blocks = data.dtypes.unique()
    # Create a dictionary to hold data blocks by type
    blocks = {}

# Get data types present in the DataFrame
    dtypes = data.dtypes.unique()

# Populate the blocks dictionary with dataframes of each type
    for dtype in dtypes:
        # Select columns of the specific dtype
        cols = data.select_dtypes(include=[dtype]).columns
        # Create a block (subset DataFrame) for each dtype
        blocks[dtype] = data[cols]
...
def blocks_by_dtype(data):
    """
    Gives column type report, returns separated blocks by data type.
    """
    # Initialize an empty dictionary to store data blocks separated by type
    blocks = {}
    print("Total Number of Columns: {}\nBreakdown....\n".format(len(data.columns)))
    
    # Get unique data types present in the DataFrame
    
    data_types = data.dtypes.unique()
    
    for dtype in data_types:
        # Select columns of each data type
        selected_data = data.select_dtypes(include=[dtype])
        # Add the selected columns to the blocks dictionary
        blocks[dtype] = selected_data
        print("Type: {} , Count: {} \nColumns and null counts---: \n{}\n".format(
            dtype, len(selected_data.columns), selected_data.isnull().sum()))
...
#               '_7_ddate.csv' , '_8_dobject.csv' ]
    ...
    for i in range(len(dnames)):
        try:
            ...
            chunky_dict[chunky_keys[i]] = pd.read_csv(f_path+dnames[i])
        except UnicodeDecodeError:
            print("Failed with encoding error, trying again for: ... ", i)
            chunky_dict[chunky_keys[i]] = pd.read_csv(f_path+dnames[i], encoding = "ISO-8859-1")
        print("Sucess for: ... ", i)
...
def generate_country_logistics_features():
    ...
    # Iterate over sheets, pull and rename
    lpi_dfs = []
    for s in lpi.sheet_names:
        df_ = lpi.parse(sheet_name=s).reset_index(drop=True)
        df_.columns = [f"{s}&{c}" if c != 'country' else 'country' for c in df_.columns]
        lpi_dfs.append(df_)

    # Consolidate country list
    lpicountries = set().union(*(df['country'] for df in lpi_dfs))
    print(len(lpicountries))

    # Create a DataFrame to merge all sheets horizontally
    ct = pd.DataFrame(lpicountries, columns=['country'])

    # Iterate and merge
    for d in lpi_dfs:
        ct = ct.merge(d, on='country', how='left')
    print("shape ct: ", ct.shape)

    # Handling missing columns by imputation
    lpi_cols = list(set(col.split('&')[1] for col in ct.columns if '&' in col))
    mis_years = ['2006', '2008', '2009', '2011', '2013', '2015']
    missing_cols = [f"{y}&{attr}" for y in mis_years for attr in lpi_cols]
    print("missing_cols:", len(missing_cols))

    # Impute missing years with calculated values
    # Define a dictionary for year mappings
    year_map = {
        '2006': '2007',
        '2008': lambda df, attr: (df[f'2010&{attr}'] - df[f'2007&{attr}']) / 3 + df[f'2007&{attr}'],
        '2009': lambda df, attr: 2 * (df[f'2010&{attr}'] - df[f'2007&{attr}']) / 3 + df[f'2007&{attr}'],
        '2011': lambda df, attr: (df[f'2010&{attr}'] + df[f'2012&{attr}']) / 2,
        '2013': lambda df, attr: (df[f'2012&{attr}'] + df[f'2014&{attr}']) / 2,
        '2015': lambda df, attr: (df[f'2014&{attr}'] + df[f'2016&{attr}']) / 2,
    }

    for y, mapping in year_map.items():
        for attr in lpi_cols:
            col_name = f"{y}&{attr}"
            if callable(mapping):
                ct[col_name] = mapping(ct, attr)
            else:
                ct[col_name] = ct[f"{mapping}&{attr}"]

    # Fill NaNs with mean
    for col in ct.columns[1:]:
        ct[col].fillna(ct[col].mean(), inplace=True)

    # Reshape for analysis
    stacked = ct.set_index('country').stack().reset_index()
    stacked['year'] = stacked['level_1'].str.split('&').str[0]
    stacked['score_type'] = stacked['level_1'].str.split('&').str[1]
    stacked.drop('level_1', axis=1, inplace=True)
    stacked.rename(columns={0: 'score'}, inplace=True)
    unstacked = stacked.pivot_table(index=['country', 'year'], columns='score_type', values='score').reset_index()

    print(unstacked.shape)
    print(unstacked.head())
    print("lpi describe: ", unstacked.describe())
...
def combine_logistics_and_stability_features(lpi_all, fsi_all):
    ...
    #import os
    #f_path = os.curdir+'\Data\Features\\'
    df_country = compare_columns(fsi_all, lpi_all,'country','country')
    df_country.to_csv(f_path+'fsi_lpi_country comps.csv')
    # After doing data cleaning in Excel, get the resulting map
    map1 = pd.ExcelFile(f_path+'fsi_lpi_country comps_map.xlsx').parse(sheetname='fsi-lpi-map')
...
def generate_factory_location_features(main_data):
    ...
    import os
    #path = os.curdir + '\Data\Features\\'
    fact_ref = pd.read_csv(f_path+'factory_map_premade.csv', encoding = "ISO-8859-1")
    #fact_ref = pd.read_excel(f_path+'factory_map_premadeX.xlsx', encoding = "ISO-8859-1")
    ...
    #import os
    #path = os.curdir+'\Data\Features\\'
    factory.to_csv(f_path+"factory_country_continent.csv")
...
def destination_and_origin_lpi_fsi_indicators(main_data, lpi_fsi_combined): # Must have origin factory/country first
    ...
    #------------------------------------------------#
    dest = main_data.groupby(['country', 'del_date_scheduled_yr'])['delayed'].agg(['count', 'sum', 'mean'])
    orig = main_data.groupby(['origin_country', 'del_date_scheduled_yr'])['delayed'].agg(['count', 'sum', 'mean'])
...
def add_lpi_fsi_features(main_data, origin_metrics, destination_metrics): # Must Happen after they are generated!
    ...
    dest = dest.groupby(['dest_country', 'dest_del_date_scheduled_yr']).agg('mean',numeric_only=True)
    orig = orig.groupby(['orig_origin_country', 'orig_del_date_scheduled_yr']).agg('mean',numeric_only=True)
...
def train_test_oversample(X, y, test_size=0.35, use_smote=False):
    ...
    smote = SMOTE(random_state=121, sampling_strategy = 1.0)
...
def visual_model_selection(X_train, X_test, y_train, y_test, estimator, show_plot=True):
    ...
    """
    Takes train and test datasets for both features and target, plus an estimator,
    and returns a visual classification report.
    """
    model = Pipeline([('estimator', estimator)])

    # Instantiate the classification model and visualizer
    visualizer = ClassificationReport(model, classes=['on-time', 'delayed'], support=True)
    ...
    if show_plot:
        ...
        visualizer.finalize()  # Finalize to make sure everything is rendered if not showing the plot immediately
...
def run_combined_classify_regress_model(data, ddate, delayed,classifier, regressor
    ...
    df_pred_fin.to_csv(r_path+'classifier_final_predicted.csv')
    df_true_pos_fin.to_csv(r_path+'classifier_final_true_positives.csv') # Save to data folders
